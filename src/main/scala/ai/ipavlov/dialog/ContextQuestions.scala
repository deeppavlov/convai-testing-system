...
object SquadQuestions extends ContextQuestions {
  private val rnd = scala.util.Random

  def selectRandom: Try[String] = {
    Try(io.Source.fromResource("context.squad.txt").getLines.size).flatMap { size =>
      val dataset = io.Source.fromResource("context.squad.txt")
